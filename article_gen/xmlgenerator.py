...
username = "Shreyateru"  # change this also
...
def main():
    ...
    pages = ""
    titlefile = "./other_files/new_titles"
    ...
    for subdomain in domains:
        xmlTextTemplate = env.get_template("telugu_hospitals.j2")
        subdomainfile = "translations/" + subdomain + "_hosp_telugu.json"
        data = pandas.read_json(subdomainfile)
        pages += xmlGenerator(titles, xmlTextTemplate, data)

    # save pages
    xmlpage = tewiki + pages + "</mediawiki>"
    # xmlpage = xmlpage.replace('&','అండ్')
    xmlpage = xmlpage.replace("మరియు", "అండ్")
    xmlpage = xmlpage.replace("యొక్క", "")
    xmlpage = xmlpage.replace("జాతీయగీతము", "ఎక్స్-రే")
    f = open("1135_hosp_tel.xml", "w")
    f.write(xmlpage)
    f.close()
