...
placetype_trans = {"city": "నగరం", "town": "పట్టణం", "village": "గ్రామం"}
trans = {
    **months_trans,
    **caretype_trans,
    **management_trans,
...
def hardcode(record):
    ...
    for col in cols:
        ...
        try:
            try:
                month_date, year = record[col].split(",")
            except:
                month_date, year = record[col].split(".")
            month, date = month_date.split(" ")
        except:
            month, date, year = record[col].split()

        if not month.isalnum():
            continue
        try:
            record[col] = trans[month.lower()] + " " + date + ", " + year
        except:
            print("month", month.lower()), print(col, record), exit()
...
all_cols = [
    ...
    "Eyebank" "Valid_From",
    "Valid_Upto",
    "Date_First" "Placetype",
    "Management",
    "Systems",
    "Caretype" "Hospital_Name",
    "Location",
    "Address",
    "District",
    "State",
