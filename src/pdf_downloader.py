...
def download_file(download_code, url):
    ...
    download_url = 'https://www.nabh.co/Documents/AccreditedList/'+url+'/'+download_code+'.pdf'
...
for index,entry in accr_hosp.iterrows():
    ...
    code = entry['Code']
    url = folder[caretype]
    if url in ['PHC','Hospitals','SHCO']: #This is because certificates are not  available for PHCS
        incorrect_files_count+=1
        continue
